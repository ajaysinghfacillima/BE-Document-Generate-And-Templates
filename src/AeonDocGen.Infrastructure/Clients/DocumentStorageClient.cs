...
/// <summary>
/// NAS/filesystem-backed durable storage client for tenant-scoped generated document file persistence.
...
/// </summary>
public sealed class DocumentStorageClient : IDocumentStorageClient
{
    ...
    {
        _basePath = settings.Value.DocumentBasePath;
    ...
    public async Task<string> StoreFileAsync(string tenantScopedPath, byte[] content, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation(
            "Document storage write starting. TenantScopedPath={TenantScopedPath}, ContentLength={ContentLength}, BasePath={BasePath}, RequestId={RequestId}",
        ...
        _logger.LogInformation(
            "Document file stored successfully. TenantScopedPath={TenantScopedPath}, StorageUri={StorageUri}, RequestId={RequestId}",
    ...
    public async Task DeleteFileAsync(string storageUri, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(
                "Document storage delete skipped for unrecognized URI scheme. StorageUri={StorageUri}, RequestId={RequestId}",
        ...
        {
            _logger.LogInformation(
                "Document file already absent. StorageUri={StorageUri}, FullPath={FullPath}, RequestId={RequestId}",
        ...
        _logger.LogInformation(
            "Document file deleted. StorageUri={StorageUri}, FullPath={FullPath}, RequestId={RequestId}",
            storageUri, fullPath, GetRequestId());
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            catch (IOException ex) when (attempt < _maxRetryAttempts)
            {
                ...
                _logger.LogWarning(
                    ex,
                    "Transient document storage I/O failure. Operation={OperationName}, TenantScopedPath={TenantScopedPath}, Attempt={Attempt}/{MaxAttempts}, RetryDelayMs={RetryDelayMs}, RequestId={RequestId}",
            ...
            catch (IOException ex)
            {
                _logger.LogError(
                    ex,
                    "Document storage operation failed after all retry attempts. Operation={OperationName}, TenantScopedPath={TenantScopedPath}, Attempts={Attempts}, RequestId={RequestId}",
